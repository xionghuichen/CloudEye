...
class YouTu(object):
    ...
    def FaceVerify(self, person_id, image_path, data_type = 0):
        ...
        if data_type == 0:
            ...
            data["image"] = data["image"] = image_path.rstrip().decode('utf-8')
    ...
    def FaceIdentify(self, group_id, image_path, data_type = 0):
        ...
        if data_type == 0:
            ...
            data["image"] = image_path.rstrip().decode('utf-8')
        if len(group_id) == 0:
    ...
    def AddFace(self, person_id, images, tag='', data_type = 0): 
        ...
        if  data_type == 0:
            ...
            images_content = []
            for image in images:
                images_content.append(image.rstrip().decode('utf-8'))
            data["images"] = images_content
        r = {}
        try:
            r = requests.post(url, headers=headers, data = json.dumps(data))
    ...
    def namecardocr(self, image_path, data_type = 0, retimage= 1, seq = ''):
